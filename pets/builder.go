...
func (p *Pet) SetAgeEstimate(s bool) *Pet {
	p.AgeEstimated = s
	return p
}
...
func (p *Pet) Build() (*Pet, error) {
	if p.MinWeight > p.MaxWeight {
		return nil, errors.New("minimum weight must be greater than max weight")
	}
